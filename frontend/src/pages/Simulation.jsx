...
export default function Simulation() {
  ...
  // Supabase Realtime: subscribe to changes on temple_simulation for near real-time updates
  useEffect(() => {
    if (!selectedTemple) return
    const isoDate = (() => {
      try { const d = new Date(selectedDate); if (!isNaN(d)) return d.toISOString().split('T')[0] } catch(_){}
      return selectedDate
    })()
    const channel = supabase
      .channel(`sim-${selectedTemple._id}-${isoDate}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'temple_simulation',
        filter: `temple_id=eq.${selectedTemple._id},date=eq.${isoDate}`,
      }, (payload) => {
        ...
        setData(prev => {
          ...
          return {
            ...prev,
            currentStatus: row.current_status || prev.currentStatus,
            areas: row.areas || prev.areas,
            facilities: row.facilities || prev.facilities,
            alerts: row.alerts || prev.alerts,
            weatherImpact: row.weather_impact || prev.weatherImpact,
            hourlyData: row.hourly_data || prev.hourlyData,
          }
        })
      })
      .subscribe()
